...
```
二进制（Binary）:
以 0b 或 0B 开头，后面跟二进制数字（仅包含 0 和 1）。
例如：0b1010 表示二进制数字 1010，即十进制的 10。

十进制（Decimal）:
默认的数字形式，直接写数字。
例如：1010 就是十进制数字 1010。

八进制（Octal）:
以 0 开头，后面跟八进制数字（仅包含 0 到 7）。
例如：01010 是八进制数字 1010，它在十进制中是 520（01010 = 8^3 * 0 + 8^2 * 1 + 8^1 * 0 + 8^0 * 1）。

十六进制（Hexadecimal）:
以 0x 或 0X 开头，后面跟十六进制数字（包含 0-9 和 A-F）。
例如：0x10101 是十六进制数字 10101，它在十进制中是 65537。
